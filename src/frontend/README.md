...
- **Cache por sessão**: o `AppProviders` escuta mudanças do Supabase Auth e, quando o usuário realmente muda (login, logout ou troca de conta), limpa o TanStack Query **e a indústria selecionada** — evitando dados de uma sessão anterior após novo login e garantindo que todo login passe pela seleção de indústria. O primeiro evento recebido é sempre tratado como linha de base, nunca como troca: o `INITIAL_SESSION` do supabase-js é emitido de forma assíncrona e um `TOKEN_REFRESHED` de rotina pode chegar antes dele — sem essa guarda, a renovação automática do token seria lida como "outro usuário" e derrubaria a sessão no meio da navegação.
...
- **Indústria em escopo (multi-acesso)**: um e-mail pode estar vinculado a vários registros de distribuidor (cada um representando distribuidor × indústria). O `IndustryProvider` (montado no layout do portal) resolve o escopo antes de qualquer tela renderizar: 1 vínculo entra direto, 2+ passam por `/selecionar-industria` e ganham o dropdown "Indústria" no topo (`IndustrySwitcher`); 0 vínculos ativos cai na tela "Sem acesso". A escolha vale para a sessão de login (sobrevive a refresh e novas abas) e é descartada no logout/troca de usuário, então cada novo login volta a passar pela seleção. Todas as consultas/escritas de usuários distribuidores carregam o id da indústria selecionada explicitamente (relatórios via `useReportFilters`, Dados via `PeriodFilterBar`, upload/planner via `useIndustryScope`); trocar de indústria remonta as telas e descarta o cache de dados. Rótulos centralizados em `lib/industry.ts`.
- **Evoluções**: Análise Mensal agrupa semanas no padrão Excel (`WEEKNUM`, semana começando no domingo) e Histórico 3 Meses usa o mês final do período filtrado como M, exibindo M-2, M-1 e M com variações M x M-2/M-1. Mark Up % e Margem % do Histórico 3 Meses seguem o racional validado de preço médio Sell Out versus preço médio Sell In no mesmo mês/filtro.
...
- **Export com feedback**: CSV com toast de sucesso/erro, seção de filtros/contexto e dados formatados como na tela. Relatórios exportam o resultado completo do filtro; telas de Dados exportam a página atual filtrada e informam página, tamanho, total filtrado e registros exportados.
