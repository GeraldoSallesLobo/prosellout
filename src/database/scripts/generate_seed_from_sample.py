#!/usr/bin/env python3
"""Generate database/supabase/seed.sql from the real import sample.
...
def main() -> None:
    ...
    out = []
    w = out.append
    w("-- AUTO-GERADO por database/scripts/generate_seed_from_sample.py")
    w("-- Fonte: .dev_files/dados-importacao (amostra real, distribuidor único, LGPD).")
    w("-- NÃO editar à mão; regenerar com o script.\n")
    ...
    w("insert into file_imports (file_name, sheet_name, file_type_id, status, total_records, processed_records, error_count, finished_at) values")
    w(",\n".join(
        f"  ({sql_str(fn)}, 'Planilha1', '{ftc_id[code]}', '{st}'::import_status, {tot}, {proc}, {err}, now() - '{i} days'::interval)"
        for i, (fn, code, st, tot, proc, err) in enumerate(imports, start=1)) + ";\n")

    w("insert into channels (id, name) values")
    w(",\n".join(f"  ('{channel_id[c]}', {sql_str(c)})" for c in channels) + ";\n")

    w("insert into clusters (id, name) values")
    w(",\n".join(f"  ('{cluster_id[c]}', {sql_str(c)})" for c in clusters) + ";\n")

    # hierarchy: macro, then cat, then sub
    w("insert into product_hierarchy (id, parent_id, level, name) values")
    lines = [f"  ('{mid}', null, 'macro_category', {sql_str(m)})" for m, mid in macros.items()]
    for c, cid in cats.items():
        macro_of = next(str(r[7]) for r in produtos if str(r[6]) == c)
        lines.append(f"  ('{cid}', '{macros[macro_of]}', 'category', {sql_str(c)})")
    for s, (sid, cat, macro) in subs.items():
        lines.append(f"  ('{sid}', '{cats[cat]}', 'subcategory', {sql_str(s)})")
    w(",\n".join(lines) + ";\n")
    ...
    # sell out — invoice_number proxy = pdv-date (a "nota" per PDV/day)
    ...
    # targets — aggregate meta by (customer, product, month)
    ...
    for r in meta:
        ...
        try:
            ...
            pass
    tgt_vals = [
        f"  ('{cid}', '{pid}', '{month}', {qty!r}, {round(gv, 2)!r})"
        for (cid, pid, month), (qty, gv) in agg.items()
    ]
    batch(
        "insert into sales_targets (customer_id, product_id, target_date, quantity, gross_value) values",
        tgt_vals,
    )
