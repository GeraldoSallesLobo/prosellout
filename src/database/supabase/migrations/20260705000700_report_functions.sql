...
begin
  ...
  -- Projection kept only for the "Tendência" KPI.
  ...
  v_pdv_count := fn_customer_count(p_channel_id, p_cluster_id, p_sales_rep_id, p_supervisor_id);
  ...
  return jsonb_build_object(
    ...
    -- Mark Up % / Margem % still use the sell_out unit_cost basis. The official
    -- reference defines them against the Sell In average price; that rework is
    -- pending a business decision (label form + how to handle dimensions that
    -- Sell In lacks). See docs/PENDENCIAS_CALCULO.md.
    ...
    -- Probabilities: raw achievement ratios (no projection, no cap).
