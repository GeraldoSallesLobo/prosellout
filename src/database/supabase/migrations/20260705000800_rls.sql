...
grant execute on function fn_customer_count to authenticated;
...
-- Table-level privileges: RLS governa as LINHAS; sem GRANT, o PostgREST devolve
-- 403 na leitura direta. As policies acima são `to authenticated`; concedemos
-- os privilégios de tabela ao mesmo role (anon não recebe nada).
...
grant select on
  sell_out, sell_in, stock_snapshots, sales_targets, file_import_logs
  to authenticated;

grant select, insert on file_imports to authenticated;
