alter table platform_data_deletion_logs
  drop constraint if exists platform_data_deletion_logs_dataset_check;

alter table platform_data_deletion_logs
  add constraint platform_data_deletion_logs_dataset_check
  check (
    dataset in (
      'customers',
      'sales_reps',
...
declare
  ...
  v_hierarchy_ids uuid[] := array[]::uuid[];
...
begin
  ...
  if v_has_selected_rows and p_dataset in (
    'customers',
    'sales_reps',
    'product_hierarchy',
    'commercial_hierarchy'
  ...
  case p_dataset
    when 'customers' then
      ...
      if cardinality(v_customer_ids) > 0 then
        ...
        into v_distributor_refs, v_entity_refs
        from customers c
    ...
    when 'product_hierarchy' then
      if not v_has_selected_rows then
        raise exception 'Selected product hierarchy rows are required';
      end if;

      with recursive hierarchy_scope as (
        select ph.id, ph.parent_id, ph.level, ph.name, ph.distributor_id
        from product_hierarchy ph
        where ph.id = any(v_selected_uuid_ids)

        union all

        select child.id, child.parent_id, child.level, child.name, child.distributor_id
        from product_hierarchy child
        join hierarchy_scope parent on parent.id = child.parent_id
      )
      select
        coalesce(array_agg(distinct scope.id), array[]::uuid[]),
        coalesce(
          jsonb_agg(distinct jsonb_build_object(
            'id', scope.id,
            'name', scope.name,
      ...
      into v_hierarchy_ids, v_entity_refs, v_distributor_refs
      from hierarchy_scope scope
      left join distributors d on d.id = scope.distributor_id;

      if cardinality(v_hierarchy_ids) > 0 then
        select coalesce(array_agg(p.id), array[]::uuid[])
        into v_product_ids
        from products p
        where p.subcategory_id = any(v_hierarchy_ids);

        if cardinality(v_product_ids) > 0 then
          delete from sales_targets st
          where st.product_id = any(v_product_ids);

          delete from sell_in_targets sit
          where sit.product_id = any(v_product_ids);

          delete from stock_snapshots ss
          where ss.product_id = any(v_product_ids);

          delete from sell_out so
          where so.product_id = any(v_product_ids);

          delete from sell_in si
          where si.product_id = any(v_product_ids);
        ...
        loop
          delete from product_hierarchy ph
          where ph.id = any(v_hierarchy_ids)
            and not exists (
              select 1
              from product_hierarchy child
              where child.parent_id = ph.id
                and child.id = any(v_hierarchy_ids)
            );

          get diagnostics v_deleted_rows = row_count;
          exit when v_deleted_rows = 0;
          v_deleted_count := v_deleted_count + v_deleted_rows;
        end loop;

        perform refresh_report_views();
      end if;

    when 'commercial_hierarchy' then
      if not v_has_selected_rows then
        raise exception 'Selected commercial hierarchy rows are required';
      end if;

      select
        coalesce(array_agg(distinct sr.id), array[]::uuid[]),
        coalesce(
          jsonb_agg(distinct jsonb_build_object(
            'id', sr.id,
            'name', sr.name,
      ...
      into v_sales_rep_ids, v_entity_refs, v_distributor_refs
      from sales_reps sr
      left join distributors d on d.id = sr.distributor_id
      where sr.id = any(v_selected_uuid_ids)
        or sr.supervisor_id = any(v_selected_uuid_ids)
        or sr.manager_id = any(v_selected_uuid_ids);
  ...
  if p_distributor_id is not null and jsonb_array_length(v_distributor_refs) = 0 then
    select coalesce(
  ...
  v_filters := jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'row_ids', p_row_ids,
      'items', case when jsonb_array_length(v_entity_refs) > 0 then v_entity_refs else null end,
      'start', p_start,
      'end', p_end,
...
begin
  ...
  set status = 'inactive'::entity_status
  where d.id = p_distributor_id
    and d.status <> 'inactive'::entity_status;
...
revoke execute on function inactivate_distributor(uuid) from public, anon;

grant execute on function delete_platform_data(
  text, text[], date, date, uuid, text, text, uuid[], uuid, uuid
) to authenticated;

grant execute on function inactivate_distributor(uuid) to authenticated;
