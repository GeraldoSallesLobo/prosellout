...
begin
  ...
  with valid_target_dates as (
    select distinct s.target_date::date as target_date
    from staging_targets s
    join lateral (
      select c.id
      from customers c
  ...
  using valid_target_dates vtd
  where st.distributor_id = v_distributor_id
    and st.target_date = vtd.target_date
  ...
  with parsed as (
    select
      ...
      ch.id as channel_id,
      cl.id as cluster_id,
      case when fn_is_iso_date(s.target_date) then s.target_date::date end as target_date,
      case
