...
- Carga em lote: ETL faz `COPY` para `staging_*` (UNLOGGED) e chama `process_*_staging(import_id)` — validação set-based, rejeições em `file_import_logs`, insert em massa. Sell Out e Sell In acumulam as partes válidas em tabelas persistentes e só substituem os dados anteriores do mesmo distribuidor quando todas as linhas do arquivo foram processadas. A ativação é atômica por arquivo, retries de uma mesma linha são ignorados e, em cargas concorrentes, prevalece em cada data o arquivo com a maior `ingestion_sequence` gerada pelo banco.
- A migration que ativa a substituição por data aborta se houver Sell Out ou Sell In com status `processing`. Aguarde essas cargas terminarem antes do `supabase db push`; imports ainda em `pending` ou `validating` podem continuar normalmente depois da migration.
- Relatórios usam `mv_sell_out_daily` para somas; cobertura (distinct clientes) é calculada na partição com índices dedicados. No **Status MTD**, valor/volume somam o intervalo filtrado. Sem SKU específico, cobertura/ticket usam os PDVs do mês inicial do período; com um ou mais SKUs selecionados, usam os PDVs únicos do intervalo. Drop Size usa `Sell Out R$ / nº de pedidos` (`count(*)` dos registros elegíveis, recorte por Data Faturamento), conforme memória de cálculo de 13/08/2026. Giro Médio usa `Volume Estoque / Volume Sell Out` e Cobertura Média usa `Volume Estoque / dias corridos do período` (Volume Estoque = Volume Sell In − Volume Sell Out do período). Mark Up usa `preço médio Sell Out / preço médio Sell In - 1`; na coluna Meta, Mark Up, Margem, Giro Médio e Cobertura Média usam `sell_in_targets` contra `sales_targets`. A Tendência Sell Out R$ usa o realizado quando o período termina no último dia do mês; antes disso, aplica o atingimento contra a meta até a data sobre a meta total do mês.
