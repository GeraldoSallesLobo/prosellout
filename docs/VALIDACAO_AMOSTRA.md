...
- **Drop Size**: atualizado em 13/08/2026 (`Memória de Cálculo_2.docx`) para `Sell Out R$ / número de pedidos/entregas`, exibido como moeda. Cada registro elegível conta uma vez (`count(*)` após filtros, recorte por Data Faturamento; Data Entrega não participa). Vale para atual, meta (`count(*)` de `sales_targets`) e ano anterior. A regra anterior de 19/07/2026 (`Volume Sell Out / Cobertura`) deixou de ser usada.
- **Giro Médio e Cobertura Média**: atualizados em 13/08/2026 — Giro Médio = `Volume Estoque / Volume Sell Out` (a fórmula que antes alimentava Cobertura Média) e Cobertura Média = `Volume Estoque / dias corridos do período` (contagem inclusiva). Volume Estoque nesses indicadores = Volume Sell In − Volume Sell Out do período, sem usar a posição acumulada de **Dados › Estoque**.
...
- **Indústria/Marca**: foi confirmado que a próxima estrutura terá `Marca` obrigatória em todos os layouts, vínculo por marca e visão "todas as marcas" somada por distribuidor. Em 13/08/2026 foi adotado o caminho operacional sem coluna `Marca` (spec `.dev_files/SPEC_LOGIN_MULTI_INDUSTRIA.md`): cada registro de distribuidor representa distribuidor × indústria e um e-mail pode acessar vários registros, com seleção pós-login e dropdown "Indústria" no portal.

## Follow-ups (não feitos ainda)

- **Indústria/Marca**: o seletor de indústria no acesso/portal foi entregue em 13/08/2026 via multi-acesso por distribuidor (sem coluna `Marca`). Segue pendente a dimensão importada: consumir a coluna `Marca` nos layouts, lista de marcas, nome/código exato, exemplos atualizados e confirmação sobre repetição de Clientes/Vendedores entre marcas. A visão "todas as marcas" somada ficou para uma fase 2 (os `report_*` já agregam com `p_distributor_id = null`).
