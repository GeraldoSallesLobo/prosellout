...
- **Meta Sell In**: `Layout SellIn_meta.xlsx` é um tipo próprio (`SELL_IN_TARGETS`) e não deve ser importado como `SELL_IN` nem como `TARGETS`. Ele alimenta a coluna **Meta** de `Mark Up %`, `Margem %` e `Giro Médio` no **Status MTD**, comparando Meta Sell Out contra Meta Sell In.
